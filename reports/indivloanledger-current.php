...
<html lang="en">
	...
	<body>
		...
		<div class="container">
			...
			<small>Loan Ledger - Individual </small><br /><small>Beginning - Current</small></h4>
			<hr class="thin bg-grayLighter">
...
<script type="text/javascript">
	...
	function fnGetIDMas(){
		...
		req.done(function(data){
			...
			$.each(data,function(key,val) {
				//toAppend+='<option name="'+val.first_name+' ' +val.middle_name+' ' +val.family_name+'" value="'+val.idnum+'">'+val.idnum+' - '+val.first_name+' ' +val.middle_name+' ' +val.family_name+'</option>';
				toAppend+='<option name="' +val.family_name+', '+val.first_name+' ' +val.middle_name+'" value="'+val.idnum+'">' +val.family_name+', '+val.first_name+' ' +val.middle_name+'</option>';
			});
	...
	function fnGetIndivLoanLedger(_params){
		$('#borrowersname').text('');
		var req=$.ajax({
			url: '../classes/BLL/IndivLoanLedger.php',
			method: 'post',
			datatype: 'json',
			data:{
				params:_params
		...
		req.done(function(data){
			...
			$('#borrowersname').text("Borrower's Name: "+$('#txtid option:selected').attr('name'));
		});
