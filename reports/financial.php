	...
	<body>
		...
		<div class="container">
			...
			<div class="grid" style="margin-bottom:120px;">
				<div id="financialresults" class="row cells12">
				</div>
				<div class="row cells12" style="margin-top:100px;">
...
<script type="text/javascript">
	$(function(){
		...
		$('#btnGenerateReports').click(function(){
			...
			//$('#thismonth').text($('#txtacmonth option:selected').text()+' '+$('#txtacyear').val());
	...
	function fnFinancialStatement(_params){
		var req=$.ajax({
			url: '../classes/BLL/FinancialStatement.php',
			method: 'post',
			datatype: 'json',
			data:{
				params:_params
		...
		req.done(function(data){
			...
			$('#financialresults table').append('<thead><tr>'+
			'<th></th>'+
			'<th></th>'+
			'<th class="place-right">This Month</th>'+
			'<th colspan=3 style="width:50px;">&nbsp</th>'+
			'<th class="place-right">Last Month</th>'+
			'</tr></thead>');
			
			var _params2={};
			_params2['action']='getreportfooter';
			_params2['reportname']='financialposition';
